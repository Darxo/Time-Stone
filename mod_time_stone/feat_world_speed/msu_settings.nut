{	// World Speed
	...
	{	// New Features
		local neverDisableSpeedButtonsCallback = function( _oldValue )
		{
			if (!::MSU.Utils.hasState("world_state")) return;	// otherwise the game crashes when changing settings in main menu
			::World.State.updateTopBarButtonState();
		}

		worldSpeedPage.addBooleanSetting("NeverDisableSpeedButtons", true, "Never disable Speed Buttons", "Vanilla disables some of the speed options during camping or escorting. By enabling this setting, all speed buttons now stay enabled at all times").addAfterChangeCallback(neverDisableSpeedButtonsCallback);
	}
}
